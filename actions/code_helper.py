...
def _get_screenshot_quality(target: str = "llm") -> int:
    try:
        _cfg_path = BASE_DIR / "config" / "api_keys.json"
        _cfg = json.loads(_cfg_path.read_text(encoding="utf-8"))
        key = "screenshot_quality_moondream" if target == "moondream" else "screenshot_quality_llm"
        return int(_cfg.get(key, 85 if target == "moondream" else 65))
    except Exception:
        return 65
...
def _resolve_save_path(output_path: str, language: str) -> Path:
    ...
    return DESKTOP / f"orthos_code{ext}"
...
def _take_screenshot() -> Path | None:
    try:
        ...
        screenshot_path = Path.home() / "Desktop" / f"orthos_debug_{int(time.time())}.png"
...
def _screen_debug_action(description, file_path, player, speak=None) -> str:
    ...
    try:
        ...
        # Compress image to JPEG for Ollama
        ...
        # Call main LLM (Ollama) directly with vision
        _cfg_path = BASE_DIR / "config" / "api_keys.json"
        _cfg = json.loads(_cfg_path.read_text(encoding="utf-8"))
        _url = _cfg.get("llm_url", "http://localhost:11434").rstrip("/")
        _model = _cfg.get("llm_model", "gemma4:31b-cloud")
