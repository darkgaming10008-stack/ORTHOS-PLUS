...
def _call_tool(tool: str, parameters: dict, speak: Callable | None) -> str:
    ...
    elif tool == "screen_locate":
        from actions.screen_processor import screen_locate
        return screen_locate(parameters=parameters, player=None)
...
def _execute_loop_step(step: dict, cancel_flag: threading.Event | None, speak: Callable | None) -> str:
    ...
    print(f"[Executor] [LOOP] Start ({max_iter} max): {desc}")

    while iteration < max_iter:
        iteration += 1
        print(f"[Executor] [LOOP] Iteration {iteration}/{max_iter}")
        ...
        for substep in substeps:
            ...
            tool   = substep.get("tool", "generated_code")
            params = substep.get("parameters", {})
            sdesc  = substep.get("description", "")

            print(f"[Executor]   |-> [{tool}] {sdesc}")
            try:
                result = _call_tool(tool, params, speak)
                last_output = str(result)
                print(f"[Executor]   └─ {last_output[:80]}")
            except Exception as e:
                print(f"[Executor]   ⚠️ Substep failed: {e}")
                last_output = f"Substep error: {e}"

        import re
        if exit_cond and re.search(exit_cond, last_output, re.IGNORECASE):
            print(f"[Executor] [LOOP] Exit condition met: {exit_cond}")
            break

        import time
        time.sleep(0.5)

    print(f"[Executor] [LOOP] Finished after {iteration} iterations")
    return f"Loop completed: {iteration} iterations. Last output: {last_output[:200]}"
        ...
        while True:
            ...
            for step in steps:
                ...
                if step.get("type") == "loop":
                    result = _execute_loop_step(step, cancel_flag, speak)
                    step_results[step_num] = result
                    completed_steps.append(step)
                    print(f"[Executor] ✅ Loop step {step_num} done: {str(result)[:100]}")
                    step_ok = True
                    continue
